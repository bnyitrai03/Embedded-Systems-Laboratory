...
module TopEntity_tb;
  ...
  initial begin
    ...
    // Move one back
    PITCH_ENC_A = 0; PITCH_ENC_B = 1; #500;
    $display("T=%0t Internal pitch count = %0d", $time, dut.pitch_decoder.count);
    ...
    $display("Expected motor values:");
    $display("yaw_dir = %0d, yaw_enable = %0d, yaw_duty = %0d", 1, 1, 1000);
    $display("pitch_dir = %0d, pitch_enable = %0d, pitch_duty = %0d", 0, 1, 500);
    $display("T=%0t Received motor control values:", $time);
    $display("yaw_dir = %0d, yaw_enable = %0d, yaw_duty = %0d", dut.yaw_dir, dut.yaw_pwm_enable, dut.yaw_pwm_duty);
    $display("pitch_dir = %0d, pitch_enable = %0d, pitch_duty = %0d", dut.pitch_dir, dut.pitch_pwm_enable, dut.pitch_pwm_duty);
    
    $display("Expected PWM output values:");
    $display("pitch: INA=%b INB=%b | yaw: INA=%b INB=%b", 1'b1, 1'b0, 1'b0, 1'b1);
    $display("T=%0t Internal PWM output values:", $time);
    $display("pitch: INA=%b INB=%b | yaw: INA=%b INB=%b", dut.PITCH_DIRA, dut.PITCH_DIRB, dut.YAW_DIRA, dut.YAW_DIRB);
