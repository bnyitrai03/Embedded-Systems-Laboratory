...
/* This file describes the typedefs that are used for integers and
   ...
   one might change the YawDouble type into some float of 4 bytes.
   ...
   In 20-sim the following definitions are used:
    ...
    typedef YawDouble double;
    typedef YawInteger int;
   
   so that 
   
    YawDouble is 8 bytes (64 bits) with a range of [-1.7E+308, 1.7E+308]
    YawInteger is 4 bytes (32 bits) with a range of [-2147483648, 2147483647]
    YawCharacter is 1 byte (8 bits) with a value of [0, 255]
    YawBoolean is 1 byte (8 bits) with a value of 0 or 1
*/ 
...
typedef double YawDouble;
typedef int YawInteger;
typedef char YawCharacter;
typedef char YawBoolean;
typedef const char* YawString;
