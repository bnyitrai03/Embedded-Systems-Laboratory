...
static int run_tracker(int argc, char *argv[], gpointer user_data)
{
    const char *camera_index = "0";
    AppData data;
    GstElement *pipeline;
    GstElement *source;
    GstElement *convert;
    ...
    if (argc > 2) {
        g_printerr("Usage: %s [camera-index]\n", argv[0]);
        return 1;
    }
    if (argc == 2) {
        camera_index = argv[1];
    ...
    pipeline = gst_pipeline_new("green-ball-mac-preview");
    ...
    if (!pipeline || !source || !convert || !scale || !capsfilter || !tee ||
        !detect_queue || !sink || !preview_queue || !preview_convert ||
        !overlay || !video_sink) {
    ...
    g_object_set(source, "device-index", atoi(camera_index), NULL);
    if (NUM_BUFFERS > 0) {
    ...
    if (!gst_element_link_many(source, convert, scale, capsfilter, tee, NULL)) {
    ...
    g_print("mac preview | camera index: %s\n", camera_index);
...
{
    return gst_macos_main(run_tracker, argc, argv, NULL);
}
